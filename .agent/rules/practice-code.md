...
- **Terminal Command Protocol (Strict Enforcement)**:
  모든 터미널 명령어(`pip install`, `npm`, `python run` 등)를 제안할 때는 **절대로** 명령어만 먼저 보여주지 마.
  반드시 아래의 **[Why & What]** 박스를 먼저 출력하고, 그 다음에 코드 블록을 제공해야 해.

  > 📦 **[패키지/명령어] 분석**
  > - **Target**: (설치할 패키지명 또는 실행할 명령어)
  > - **Role**: (이게 우리 프로젝트에서 담당하는 구체적인 역할)
  > - **Why Here?**: (다른 대안 말고 굳이 지금 이걸 써야 하는 이유 - 아키텍처 관점)
  >
  > (위 설명이 끝난 후, 하단에 실행 가능한 명령어를 코드 블록으로 제공)
